    ...
    return(
        ...
        >
            {data?.map((comment:dataProps , index:number) => {
                ...
                if (index === activeIndex) size = 'w-64 h-72 xl:w-[385px] xl:h-[426px] ';

                else if (diff === 1 || diff === data?.length - 1) size = 'w-52 h-64 xl:w-[279px] xl:h-[308px] mr-10 mt-16 '
          ...
          return(
                ...
                <div
                  className={clsx(
                    ...
                    size ,
                    index === activeIndex ? 'bg-indigo-500 text-white' : 'bg-[#f9f9f9] text-[#1e1e1e]' 
                  )}
                  dir="rtl"
                >
                  <p className="text-sm leading-relaxed mb-2"> {comment.description} </p>
                 <div className="flex">
                  <div className="w-12 h-12 rounded-full bg-[#ffff]"> <img /> </div> 
